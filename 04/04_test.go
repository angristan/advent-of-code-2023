...
func TestConvertInputToListOfCards(t *testing.T) {
	...
	want := ElfStack{
		{ID: 1, WinningNumbers: []CardNumber{41, 48, 83, 86, 17},
			MyNumbers: []CardNumber{83, 86, 6, 31, 17, 9, 48, 53}},
		{ID: 2, WinningNumbers: []CardNumber{13, 32, 20, 16, 61},
			MyNumbers: []CardNumber{61, 30, 68, 82, 17, 32, 24, 19}},
		{ID: 3, WinningNumbers: []CardNumber{1, 21, 53, 59, 44},
			MyNumbers: []CardNumber{69, 82, 63, 72, 16, 21, 14, 1}},
		{ID: 4, WinningNumbers: []CardNumber{41, 92, 73, 84, 69},
			MyNumbers: []CardNumber{59, 84, 76, 51, 58, 5, 54, 83}},
		{ID: 5, WinningNumbers: []CardNumber{87, 83, 26, 28, 32},
			MyNumbers: []CardNumber{88, 30, 70, 12, 93, 22, 82, 36}},
		{ID: 6, WinningNumbers: []CardNumber{31, 18, 13, 56, 72},
...
func TestComputeMatchCount(t *testing.T) {
	type testInput struct {
		card      Card
		wantCount int
	}
	...
	input := []testInput{
		{card: Card{WinningNumbers: []CardNumber{41, 48, 83, 86, 17},
			MyNumbers: []CardNumber{83, 86, 6, 31, 17, 9, 48, 53}}, wantCount: 4},
		{card: Card{WinningNumbers: []CardNumber{13, 32, 20, 16, 61},
			MyNumbers: []CardNumber{61, 30, 68, 82, 17, 32, 24, 19}}, wantCount: 2},
		{card: Card{WinningNumbers: []CardNumber{1, 21, 53, 59, 44},
			MyNumbers: []CardNumber{69, 82, 63, 72, 16, 21, 14, 1}}, wantCount: 2},
		{card: Card{WinningNumbers: []CardNumber{41, 92, 73, 84, 69},
			MyNumbers: []CardNumber{59, 84, 76, 51, 58, 5, 54, 83}}, wantCount: 1},
		{card: Card{WinningNumbers: []CardNumber{87, 83, 26, 28, 32},
			MyNumbers: []CardNumber{88, 30, 70, 12, 93, 22, 82, 36}}, wantCount: 0},
		{card: Card{WinningNumbers: []CardNumber{31, 18, 13, 56, 72},
			MyNumbers: []CardNumber{74, 77, 10, 23, 35, 67, 36, 11}}, wantCount: 0},
	}

	for _, ti := range input {
		assert.Equal(t, ti.wantCount, ti.card.ComputeMatchCount())
	}
}

func TestComputeTotalCardsCount(t *testing.T) {
	type testInput struct {
		elfStack         ElfStack
	...
	for _, ti := range input {
		assert.Equal(t, ti.wantTotalMatches, ti.elfStack.ComputeTotalCardsCount())
	}
}
